...
async function getActiveUserDevicesOptimized(userId: string) {
  try {
    ...
    console.error("[getActiveUserDevicesOptimized] Database error:", error);
...
export async function createDeviceFingerprint(
  ...
  try {
    console.log("[createDeviceFingerprint] Starting for user:", userId);
    validateDeviceData(deviceData);
    ...
    console.log("[createDeviceFingerprint] Similar device check completed");
    ...
    console.log("[createDeviceFingerprint] Transaction completed successfully");

    revalidateTag("user-devices");
    return result;
  } catch (error) {
    console.error("[createDeviceFingerprint] Error:", error);
    throw error;
  }
}
